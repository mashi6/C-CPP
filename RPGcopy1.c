...
//追加
struct BattleField *FieldP = &Field;
struct rowColor *FieldColorP = FieldColor;
struct EnemyStatus *EnemiesP = Enemies;
struct AllyStatus *useCharactarsP = useCharactars;
//追加
...
void printStatus(struct AllyStatus charactar){
		struct AllyStatus *charactarP = &charactar;
		printf("\n");
	 	switch(charactarP->rarity){
	 		case superRare:
	 			printf("\x1b[41m");
	 			printf("---------[SuperRare]---------");
	 			break;
 		...
 		printf("　　　　　体力 : ");
 		if(charactarP->hitPoint > 85){
 			printf("\x1b[31m");
 		}else if(charactarP->hitPoint < 70){
 			printf("\x1b[36m");
 		}
 		printf("%d /100\n",charactarP->hitPoint);
 		...
 		printf("　　　　攻撃力 : ");
 		if(charactarP->attack > 25){
 			printf("\x1b[31m");
 		}else if (charactarP->attack < 15){
 			printf("\x1b[36m");
 		}
 		printf("%d /30\n",charactarP->attack);
 		printf("\x1b[39m\x1b[0m");
 		...
 		printf("　　　　防御力 : ");
 		if(charactarP->defence > 25){
 			printf("\x1b[31m");
 		}else if(charactarP->defence < 15){
 			printf("\x1b[36m");
 		}
 		printf("%d /30\n",charactarP->defence);
 		printf("\x1b[39m\x1b[0m");
 		...
 		printf("　　　　素早さ : ");
 		if(charactarP->speed > 25){
 			printf("\x1b[31m");
 		}else if(charactarP->speed < 15){
 			printf("\x1b[36m");
 		}
 		printf("%d /30\n",charactarP->speed);
 		printf("\x1b[39m\x1b[0m");
...
void printField(void){
	...
	for(int i=0;i<Max;i++){
		printf("┃");
		printf("%s",(&((FieldP->row)[i]))->margin1);

		textColor_is((&((FieldColorP+i)->AllyHP))->textColor);
		backgroundColor_is((&((FieldColorP+i)->AllyHP))->backgroundColor);
		printf("%s\x1b[49m\x1b[39m\x1b[0m",(&((FieldP->row)[i]))->AllyHP);
		printf("%c",(&((FieldP->row)[i]))->margin2);

		textColor_is((&((FieldColorP+i)->AllyName))->textColor);
		backgroundColor_is((&((FieldColorP+i)->AllyName))->backgroundColor);
		printf("%s\x1b[49m\x1b[39m\x1b[0m",(&((FieldP->row)[i]))->Allyname);
		printf("%s",(&((FieldP->row)[i]))->margin3);

		textColor_is((&((FieldColorP+i)->EnemyName))->textColor);
		backgroundColor_is((&((FieldColorP+i)->EnemyName))->backgroundColor);
		printf("%s\x1b[49m\x1b[39m\x1b[0m",(&((FieldP->row)[i]))->Enemyname);
		printf("%c",(&((FieldP->row)[i]))->margin4);

		textColor_is((&((FieldColorP+i)->EnemyHP))->textColor);
		backgroundColor_is((&((FieldColorP+i)->EnemyHP))->backgroundColor);
		printf("%s\x1b[49m\x1b[39m\x1b[0m",(&((FieldP->row)[i]))->EnemyHP);
		printf("%s",(&((FieldP->row)[i]))->margin5);
		printf("┃");
		printf("\n");
	}
	printf("┗");
...
void setupField(void){
	for(int i=0;i<Max;i++){//margin,HP 初期化
		strcpy((&((FieldP->row)[i]))->margin1,"     ");
		(&((FieldP->row)[i]))->margin2 = ' ';
		(&((FieldP->row)[i]))->margin4 = ' ';
		strcpy((&((FieldP->row)[i]))->margin5,"     ");
		if(i < charnumber){
			sprintf((&((FieldP->row)[i]))->AllyHP,"%d/%d",AllyHP[i],(useCharactarsP+i)->hitPoint);
		}
		int count = 5;
		for(int j=strlen((&((FieldP->row)[i]))->AllyHP);j<=7;j++){
			((&((FieldP->row)[i]))->margin1)[count] = ' ';
			((&((FieldP->row)[i]))->margin1)[count+1] = '\0';
			count++;
		}

		if(i < enemynumber){
			sprintf((&((FieldP->row)[i]))->EnemyHP,"%d/%d",EnemyHP[i],(EnemiesP+i)->hitPoint);
		}
		count = 5;
		for(int j=strlen((&((FieldP->row)[i]))->EnemyHP);j<=7;j++){
			((&((FieldP->row)[i]))->margin5)[count] = ' ';
			((&((FieldP->row)[i]))->margin5)[count+1] = '\0';
			count ++;
		}
	...
	for(int i=0;i<Max;i++){//Name,margin　初期化
		...
		if(i < charnumber){
			strcpy((&((FieldP->row)[i]))->Allyname,(useCharactarsP+i)->name);
		}

		if(i < enemynumber){
			strcpy((&((FieldP->row)[i]))->Enemyname,(EnemiesP+i)->name);
		}
		int count = 0;

		for(int j=((strlen((&((FieldP->row)[i]))->Enemyname) + strlen((&((FieldP->row)[i]))->Allyname))* 2 / 3);j<=34;j++){
				((&((FieldP->row)[i]))->margin3)[count] = '\x0020';
				((&((FieldP->row)[i]))->margin3)[count+1] = '\0';
				count++;
		}
