...
void setupField(void){
	for(int i=0;i<Max;i++){//margin,HP 初期化
		...
		if(i < charnumber){
			sprintf((*(&((*FieldP).row[i]))).AllyHP,"%d/%d",AllyHP[i],(*(useCharactars+i)).hitPoint);
		}
		int count = 5;
		...
		if(i < enemynumber){
			sprintf((*(&((*FieldP).row[i]))).EnemyHP,"%d/%d",EnemyHP[i],(*(Enemies+i)).hitPoint);
		}
		count = 5;
	...
	for(int i=0;i<Max;i++){//Name,margin　初期化
		...
		if(i < charnumber){
			strcpy((*(&((*FieldP).row[i]))).Allyname,(*(useCharactars+i)).name);
		}

		if(i < enemynumber){
			strcpy((*(&((*FieldP).row[i]))).Enemyname,(*(Enemies+i)).name);
		}
		int count = 0;
