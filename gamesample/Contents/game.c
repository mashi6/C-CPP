...
char map[20][40];
int check[20][20];//visited 0  yet 1
int cell[400][2];
...
void goal(void){
	...
	system("mpv \"goal.mp4\" --window-scale=1.5");
}

void die(void){
	system("stty echo -icanon min 1 time 0");
	system("mpv \"die.mp4\" --window-scale=3.5");
}
...
void make_rate(void){
	char a[20];
	sprintf(a," FINAL RATE : %0.1f",(float)score/(float)moves*(float)(bomb_num*10-((int)time(NULL)-start)));
	make_data(8,21,a);
	return;
}
void make_time(void){
